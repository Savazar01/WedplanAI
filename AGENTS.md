...
- **Terminology — Ceremonies vs Rituals**: The word `rituals` is deprecated across all code, APIs, and UI. Always use `ceremonies` or `Ceremonies`. The DB table is `rituals` (legacy) but all references in the codebase, routes, and UI must use `ceremonies`.
