...
const rewrite = (filePath, isDashboard) => {
  let content = fs.readFileSync(filePath, 'utf8');

  // We need to wrap the sections in a function: \`function getSections(t) { return [...] }\`
  content = content.replace("const sections = [", "const getSections = (t: (key: string) => string) => [");
  content = content.replace("const personasSection = {", "const getPersonasSection = (t: (key: string) => string) => ({");
  content = content.replace("const apiSection = {", "const getApiSection = (t: (key: string) => string) => ({");
  
  // End of sections array
  content = content.replace("];\\n\\nconst personasSection", "];\\n\\nconst getPersonasSection");
  content = content.replace("  ],\\n};\\n\\nconst apiSection", "  ],\\n});\\n\\nconst getApiSection");
  content = content.replace("  ],\\n};\\n\\ninterface DocSection", "  ],\\n});\\n\\ninterface DocSection");

  // allSections -> getAllSections(t)
  content = content.replace("const allSections: DocSection[] = [", "const getAllSections = (t: (key: string) => string): DocSection[] => [");
  content = content.replace("  sections[0], // getting-started", "  getSections(t)[0], // getting-started");
  content = content.replace("  personasSection,", "  getPersonasSection(t),");
  content = content.replace(/  sections\[(\d+)\], \/\//g, "  getSections(t)[$1], //");
  content = content.replace("  apiSection,", "  getApiSection(t),");

  // In the component: 
  content = content.replace("{allSections.map((section)", "{getAllSections(t).map((section)");
  // Second occurrence of allSections.map
  content = content.replace("{allSections.map((section)", "{getAllSections(t).map((section)");

  for(const [key, value] of Object.entries(enDocs)) {
    if (isDashboard && key.endsWith("contentPublic")) continue;
    if (!isDashboard && key.endsWith("contentDashboard")) continue;
    
    // special handling for small texts like "Admin" which might conflict
    if (key === "docs.adminBadge") continue; // We'll handle this manually or ignore
    
    // We use JSON.stringify to match exact strings including quotes
    const searchString = JSON.stringify(value);
    const replaceString = `t("${key}")`;
    
    // For JSX, we might have {"text"} or >text< or whatever, but in the object it's "text": "text"
    if (content.includes(searchString)) {
        content = content.replace(searchString, replaceString);
    }
  }
  
  // Handle specific JSX texts:
  content = content.replace(">Admin<", `>{t("docs.adminBadge")}<`);
  content = content.replace("WedPlanAI User Guide", `{t("docs.title")}`);
  content = content.replace(
  ...
  content = content.replace(">On this page<", `>{t("docs.onThisPage")}<`);
  content = content.replace("Need help? Contact your wedding administrator or visit the", `{t("docs.needHelp")}`);
  content = content.replace(">Settings<", `>{t("docs.settingsLink")}<`);
  content = content.replace("page for account support.", `{t("docs.pageForSupport")}`);
  
  if (!content.includes("const t = ")) {
      content = content.replace("const locale = await getLocaleServer();", 
  ...
  if (content.includes("const tDict = ")) {
      content = content.replace(/const tDict = .+;/, "");
  }
  if (content.includes("{tDict.docs}")) {
      content = content.replace("{tDict.docs}", `{t("docs.title")}`); // or actually remove that since we don't have docs keyword, wait in dashboard it was tDict.docs
  }

  // Also import `getLocaleServer` and `translations` if missing
  if (!content.includes("getLocaleServer")) {
...
rewrite('src/app/docs/page.tsx', false);
rewrite('src/app/dashboard/docs/page.tsx', true);
