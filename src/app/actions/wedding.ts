...
export async function getPublicTraditions() {
  try {
    const list = await db.select().from(weddingTraditions).orderBy(weddingTraditions.name);
    if (list.length === 0) {
      ...
      await db.insert(weddingTraditions).values(defaultTraditions);
...
export async function getPublicCategories() {
  try {
    const list = await db.select().from(taskCategories).orderBy(taskCategories.name);
    if (list.length === 0) {
      ...
      await db.insert(taskCategories).values(defaultCategories);
